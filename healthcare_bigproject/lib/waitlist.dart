...
class _ReservationsState extends State<Reservations> {
  ...
  getList() async {
    ...
    var reservations = await firebase.collection('reservation').doc(uid.toString()).get();
    ...
    for (var i = 0; i < reservations['info'].length; i++){
      var doctors = await firebase
          ...
          .get();
      var order = doctors['doctor'][reservations['info'][i]['doctor']]['waitList']
          .indexOf(uid.toString());
      print(order.runtimeType);
      ls.add({'Hospital': reservations['info'][i]['hospital'].toString(), 'Doctor': reservations['info'][i]['doctor'].toString(), 'Queue':order});
    }
...
class waitentry extends StatelessWidget {
  const waitentry({Key? key, this.i, this.ls, this.refreshFunc}) : super(key: key);
  final i;
  final ls;
  final refreshFunc;
  @override
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          OutlinedButton(style: ButtonStyle(backgroundColor: MaterialStateProperty.resolveWith<Color?>(
          ...
          ), child: Text('취소하기'), onPressed: ()async{
            var data = await firebase.collection('hospital').doc(ls[i]['Hospital']).get();
            print(data['doctor']);
            ...
            print(temp);
            await firebase.collection('hospital').doc(ls[i]['Hospital']).update({'doctor':temp});
            var reservation = await firebase.collection('reservation').doc(auth.currentUser?.uid).get();
            print(reservation['info']);
            var tempLs = reservation['info'];
            print(tempLs.runtimeType);
            tempLs.removeWhere((item) => item['doctor'] == ls[i]['Doctor'] && item['hospital'] == ls[i]['Hospital']);
            print(tempLs);
            await firebase.collection('reservation').doc(auth.currentUser?.uid).update({'info': tempLs});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          (w == '000') ? Text('${w} : 진행 중  ',style: TextStyle(fontSize: 10)) : Text('${w} : 대기 중  ',style: TextStyle(fontSize: 10)),
          //대기중, 완료 등 색상 if문 처리
          (w == '000') ? Icon(Icons.circle, size: 8, color: Colors.green,) : Icon(Icons.circle, size: 8, color: Colors.red)
        ],
      ),
    );
  }
